...
function drawRoundedRect(cr, rect, radius, fillColor, strokeColor, drawBorder = true) {
    let { x, y, width, height } = rect;

    if (drawBorder) {
        // Full rounded rect with border (editor mode)
        cr.newPath();
        ...
        // Fast plain rect fill (snapping mode)
        cr.setSourceRGBA(fillColor.r, fillColor.g, fillColor.b, fillColor.a);
...
function drawLayout(cr, node, displayRect, colors = DefaultColors, cornerRadius = 10, zoneCounter = { count: 0 }, showInfo = false) {
    if (!node) return;
    ...
    if (node.isLeaf()) {
        ...
        drawRoundedRect(cr, regionRect, cornerRadius, c, colors.border, showInfo);

        // Only draw zone numbers, dimensions, and gap info in editor mode
        if (showInfo) {
    ...
    for (let child of node.children) {
        drawLayout(cr, child, displayRect, colors, cornerRadius, zoneCounter, showInfo);
    }
}
